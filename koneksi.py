...
def cetak2():
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT id_transaksi, id_pasien, id_rawat, total_biaya, status_pembayaran, tanggal_transaksi FROM tarnsaksi_dimas ORDER BY id_transaksi ASC")
        data = cursor.fetchall()
        cursor.close()
        conn.close()
        ...
        pdf.set_font("Arial", size=14)
        pdf.cell(0, 10, txt="Laporan data Transaksi", ln=1, align='C')
        pdf.ln(5)

        pdf.set_font("Arial", size=10)
        pdf.set_fill_color(220, 220, 220)
        pdf.cell(40, 10, "ID Transaksi", border=1, fill=True)
        pdf.cell(60, 10, "ID Pasien", border=1, fill=True)
        pdf.cell(60, 10, "ID Rawat", border=1, fill=True)
        pdf.cell(30, 10, "Total Biaya", border=1, fill=True)
        pdf.cell(30, 10, "Status", border=1, fill=True)
        pdf.cell(30, 10, "Tanggal", border=1, fill=True)
        pdf.ln()

        pdf.set_fill_color(255, 255, 255)
        for d in data:
            pdf.cell(40, 10, str(d['id_transaksi']), 1, fill=True)
            pdf.cell(60, 10, str(d['id_pasien']), 1, fill=True)
            pdf.cell(60, 10, str(d['id_rawat']), 1, fill=True)
            pdf.cell(30, 10, str(d['total_biaya']), 1, fill=True)
            pdf.cell(30, 10, str(d['status_pembayaran']), 1, fill=True)
            pdf.cell(30, 10, str(d['tanggal_transaksi']), 1, fill=True)
            pdf.ln()
